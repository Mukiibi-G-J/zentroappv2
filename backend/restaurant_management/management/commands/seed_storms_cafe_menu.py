"""
Seed The Storms Cafe menu: POS Items, MenuCategories, MenuItems, Menu, and display groups.

Prerequisites:
  python manage.py tenant_command seed_restaurant_module --schema=YOUR_SCHEMA

Usage:
  python manage.py tenant_command seed_storms_cafe_menu --schema=YOUR_SCHEMA
  python manage.py tenant_command seed_storms_cafe_menu --schema=YOUR_SCHEMA --dry-run
  python manage.py tenant_command seed_storms_cafe_menu --schema=YOUR_SCHEMA --clear

Data source: restaurant_management/data/storms_cafe_menu.py
JSON export: data/restaurant/storms_cafe_menu.json (run with --export-json)
"""
...
from restaurant_management.data.storms_cafe_menu import build_catalog
...
def _full_description(item_data: dict) -> str:
    return (item_data.get("description") or "").strip()
...
@transaction.atomic
def seed_storms_cafe_menu(*, clear: bool = False, dry_run: bool = False) -> dict:
...
class Command(BaseCommand):
    help = (
        "Seed The Storms Cafe menu items (POS Items + MenuCategories + MenuItems). "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--clear",
            action="store_true",
            help="Remove existing Storms Cafe menu rows before seeding.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--export-json",
            action="store_true",
            help="Write data/restaurant/storms_cafe_menu.json from the Python catalog.",
        )
    ...
    def handle(self, *args, **options):
        ...
        def run():
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{prefix}Storms Cafe menu seed complete: "
